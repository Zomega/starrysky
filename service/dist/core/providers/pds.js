...
import { getGridDataForRange } from "../streak-logic.js";
export class PdsDataProvider extends StreakDataProvider {
  ...
  async listRecords(repo, collection) {
    ...
    return data.records.map((r) => ({ ...r.value, cid: r.cid, uri: r.uri }));
  }
  async getProfileStreaks(actor) {
    ...
    return subjects.map((subject) => {
      const subCheckins = checkins
        .filter((c) => c.subject === subject)
        .sort(
          (a, b) =>
            new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(),
        );
      const subInventories = inventories
  ...
  async getStreakDetail(actor, subject, policyUri) {
    ...
    const policy = policies.find((p) => p.uri === policyUri) || policies[0];
    const latestInventory = subInventories[subInventories.length - 1];
    const end = new Date();
